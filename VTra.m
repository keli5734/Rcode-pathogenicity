...
k2 = 0.4; %5e-5; % The maximal rate of MR -> M2 
...
beta_HP_sorted = sort(beta_HP);
quantile_range = linspace(0.025, 0.975, 10);

 for i = 1:length(quantile_range)
  beta(i) = beta_HP_sorted(round(quantile_range(i) * len_para));
  para_indx(i) = find(beta_HP == beta(i));
 end 
...
[~,y_inf_HP] = ode15s(@Model, report_time_inf, init_inf, options,...
                     ...
                     V_traj_HP(:,i) = y_inf_HP(:,6);
                      
                     % check 
                     figure(2)
                     semilogy(report_time_inf, y_inf_HP(:,6))
                     hold on 
end 
...
beta_LP_sorted = sort(beta_LP);
 for i = 1:length(quantile_range)
  beta(i) = beta_LP_sorted(round(quantile_range(i) * len_para));
  para_indx(i) = find(beta_LP == beta(i));
 end 
...
[~,y_inf_LP] = ode15s(@Model, report_time_inf, init_inf, options,...
                     ...
                     V_traj_LP(:,i) = y_inf_LP(:,6);
                      
                     % check 
                     figure(1)
                     semilogy(report_time_inf, y_inf_LP(:,6))
                     hold on 
...
writematrix(V_traj_HP,'V_traj_HP_H5N1.xls');
writematrix(V_traj_LP,'V_traj_LP_H5N1.xls');
