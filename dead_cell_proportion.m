...
Target_cell_HP = zeros(len_para, len_inf);
Target_cell_LP = zeros(len_para, len_inf);
...
[~,y_inf_HP] = ode15s(@Model, report_time_inf, init_inf, options,...
 ...
 Target_cell_HP(i,:) = (y_inf_HP(:,4) + y_inf_HP(:,8))/ T_max;
...
[~,y_inf_LP] = ode15s(@Model, report_time_inf, init_inf, options,...
 ...
 Target_cell_LP(i,:) = (y_inf_LP(:,4) + y_inf_LP(:,8))/T_max;
     
end 



writematrix(Target_cell_HP, 'Target_cell_HP_H5N1.xls')
writematrix(Target_cell_LP, 'Target_cell_LP_H5N1.xls')


%%

Target_HP = readtable('Target_cell_HP_H5N1.xls', 'ReadVariableNames', false);
Target_LP = readtable('Target_cell_LP_H5N1.xls', 'ReadVariableNames', false);
%%
interval  = 0.05:0.05:0.95;

Target_HP_quantile = quantile(table2array(Target_HP), interval, 1);
Target_LP_quantile = quantile(table2array(Target_LP), interval, 1);

%%
writematrix(Target_HP_quantile','Target_HP_quantile_H5N1.xls') 
writematrix(Target_LP_quantile','Target_LP_quantile_H5N1.xls') 
