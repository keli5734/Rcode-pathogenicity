...
function [PRCC,p_values] = my_PRCC_PLOT2(X,Y,PRCC_var,y_var)
...
for i=1:k
    ...
    a = ['PRCC = '  num2str(round(r,3)) ];% ' Time point=' num2str(s-1)];

%     figure,plot((eval(c4)),(eval(c5)),'.','MarkerEdgeColor',[.61 .51 .74],'MarkerSize',12),title(a),...
%             xlabel(PRCC_var{i}),ylabel(y_var), set(gca,'FontSize',24);%eval(c6);
